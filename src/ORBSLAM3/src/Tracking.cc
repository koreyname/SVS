    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    LoadPatchDefenseConfig(mPatchConfig);
    SetPatchDefenseConfig(mPatchConfig);
    mpPatchDefense.reset(new PatchDefense());
    mpPatchDefense->SetConfig(mPatchConfig);
    mbUsePatchDefense = mPatchConfig.use_patch_defense;
    mbStoreKeyFrameDebugImages = mPatchConfig.use_patch_defense && !mPatchConfig.loop_match_output_dir.empty();
    mMotionGuard.SetConfig(mPatchConfig.motion_guard);
    const bool orbslam2LostPolicyActive = mbUsePatchDefense && mPatchConfig.defense_orbslam2_lost_policy;
    std::cout << "[DefensePolicy] use_patch_defense=" << static_cast<int>(mbUsePatchDefense)
              << " orbslam2_lost_policy=" << static_cast<int>(mPatchConfig.defense_orbslam2_lost_policy)
              << " active=" << static_cast<int>(orbslam2LostPolicyActive) << std::endl;
}
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if(mpPatchDefense && mbUsePatchDefense && mCurrentFrame.N>0)
    {
        mpPatchDefense->ProcessFrame(mCurrentFrame, mImGray);
        UpdateRiskVisualization(mCurrentFrame);
        UpdateRiskMaskVisualization();
    }
    else
    {
        std::lock_guard<std::mutex> lock(mMutexRiskImage);
        mLastRiskImage.release();
        mbRiskImageReady = false;
        std::lock_guard<std::mutex> lock2(mMutexRiskMask);
        mLastRiskMaskImage.release();
        mbRiskMaskReady = false;
    }

#ifdef REGISTER_TIMES
    // timing already recorded above
#endif
    ...
    if(mbUsePatchDefense)
    {
        if(mCurrentFrame.N>0 && !mCurrentFrame.mK.empty())
        {
            mLastFrameForMotionGuard = Frame(mCurrentFrame);
            mLastGrayForMotionGuard = mImGray.clone();
            mbHasLastFrameForMotionGuard = true;
            mbHasLastGrayForMotionGuard = true;
        }
        else
        {
            mLastFrameForMotionGuard = Frame();
            mLastGrayForMotionGuard.release();
            mbHasLastFrameForMotionGuard = false;
            mbHasLastGrayForMotionGuard = false;
        }
    }
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        bool bOK;
        const bool useOrbSlam2LostPolicy = mbUsePatchDefense && mPatchConfig.defense_orbslam2_lost_policy;
        const int lostResetKfThreshold = useOrbSlam2LostPolicy ? 5 : 10;

        mbMotionGuardFreezePose = false;
        const bool guardEnabled = mbUsePatchDefense && mPatchConfig.motion_guard.enable_motion_guard && mState==OK;
        const bool guardHasPrev = mbHasLastGrayForMotionGuard && !mLastGrayForMotionGuard.empty() && mbHasLastFrameForMotionGuard;
        const bool guardMapReady = pCurrentMap && (pCurrentMap->KeyFramesInMap() >= std::max(0, mPatchConfig.motion_guard.min_keyframes));
        if(guardEnabled && guardHasPrev && guardMapReady)
        {
            mbMotionGuardFreezePose = mMotionGuard.Update(mLastGrayForMotionGuard, mLastFrameForMotionGuard, mImGray);
            if(mPatchConfig.motion_guard.log_motion_guard || mPatchConfig.logging)
            {
                const MotionGuardStats &st = mMotionGuard.GetLastStats();
                std::cout << "[MotionGuard] Frame " << mCurrentFrame.mnId
                          << " active=" << static_cast<int>(st.active)
                          << " trigger=" << static_cast<int>(st.trigger)
                          << " safe=" << st.tracked_safe
                          << " sus=" << st.tracked_suspect
                          << " total=" << st.tracked_total
                          << " medSafe/medSus=" << st.median_flow_safe_px
                          << "/" << st.median_flow_suspect_px
                          << " mismatch=" << st.mismatch_px
                          << " movingRatio=" << st.moving_ratio_suspect
                          << " q10/q50/q90=" << st.q10_flow_px
                          << "/" << st.q50_flow_px
                          << "/" << st.q90_flow_px
                          << " movingRatioAll=" << st.moving_ratio_total
                          << std::endl;
            }
        }
        else
        ...
        {
            if(mbHasLastGoodPose)
                mCurrentFrame.SetPose(mLastGoodPose);
            else if(mLastFrame.isSet())
                mCurrentFrame.SetPose(mLastFrame.GetPose());
            else
                mCurrentFrame.SetPose(Sophus::SE3f());
            mVelocity = Sophus::SE3f();
            mbVelocity = false;
            bOK = true;
        }
        else if(!mbOnlyTracking)
        {
            ...
            else
            {
                ...
                {
                    if(useOrbSlam2LostPolicy)
                    {
                        bOK = Relocalization();
                        if(!bOK && pCurrentMap->KeyFramesInMap() <= lostResetKfThreshold)
                        {
                            mpSystem->ResetActiveMap();
                            Verbose::PrintMess("Track lost soon after initialisation, reseting current map...", Verbose::VERBOSITY_NORMAL);
                            return;
                        }
                    }
                    else
        ...
        if(mbMotionGuardFreezePose && mpLastGoodReferenceKF)
            mpReferenceKF = mpLastGoodReferenceKF;
        ...
        if(!mbMotionGuardFreezePose && !mbOnlyTracking)
        {
            if(bOK)
                bOK = TrackLocalMap();
            if(!bOK)
        ...
        else if(!mbMotionGuardFreezePose)
        {
        ...
        {
            // Update motion model
            if(!mbMotionGuardFreezePose && mLastFrame.isSet() && mCurrentFrame.isSet())
            {
...
#endif
            bool bNeedKF = !mbMotionGuardFreezePose && NeedNewKeyFrame();
        ...
        if(mState==LOST)
        {
            if(pCurrentMap->KeyFramesInMap()<=lostResetKfThreshold)
            {
                mpSystem->ResetActiveMap();
                return;
            }
            ...
            Verbose::PrintMess("ORBSLAM2 lost policy active, skip atlas map creation.", Verbose::VERBOSITY_DEBUG);
        }

        if(!mCurrentFrame.mpReferenceKF)
            mCurrentFrame.mpReferenceKF = mpReferenceKF;

        // Always roll forward last frame, even during guard freeze.
        // This avoids resuming from a stale frame after freeze release.
        mLastFrame = Frame(mCurrentFrame);
        if(!mbMotionGuardFreezePose && mCurrentFrame.isSet())
        {
            mLastGoodPose = mCurrentFrame.GetPose();
            mbHasLastGoodPose = true;
            mpLastGoodReferenceKF = mpReferenceKF;
        }
    }
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        if(mbStoreKeyFrameDebugImages)
            pKFini->SetDebugImageGray(mImGray);
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        const bool bUseFrontendWeights = cfg.use_patch_defense && cfg.enable_weighted_frontend;
        if(bUseFrontendWeights)
        {
            matchWeights.assign(mvIniMatches.size(), 1.f);
            for(size_t i=0; i<mvIniMatches.size(); ++i)
            {
                const int j = mvIniMatches[i];
                if(j < 0)
                    continue;
                const float w1 = mInitialFrame.GetKeypointWeight(i);
                const float w2 = mCurrentFrame.GetKeypointWeight(static_cast<size_t>(j));
                matchWeights[i] = std::max(std::min(w1,w2), cfg.w_min);
            }
        }
        const std::vector<float> *pMatchWeights = bUseFrontendWeights ? &matchWeights : nullptr;

        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Tcw,mvIniP3D,vbTriangulated,pMatchWeights))
        {
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    if(mbStoreKeyFrameDebugImages)
    {
        if(!mImGrayInitial.empty())
            pKFini->SetDebugImageGray(mImGrayInitial);
        pKFcur->SetDebugImageGray(mImGray);
    }
...
{
    if(mbUsePatchDefense && mPatchConfig.motion_guard.enable_motion_guard &&
       mMotionGuard.IsActive() && mPatchConfig.motion_guard.mode >= 1)
        return false;
    ...
    // Safety check from PatchDefense (optional)
    if(mbUsePatchDefense && mpPatchDefense && mPatchConfig.keyframe_use_safe_ratio)
    {
...
void Tracking::Reset(bool bLocMap)
{
    ...
    mbHasLastGrayForMotionGuard = false;
    mbMotionGuardFreezePose = false;
    mLastGoodPose = Sophus::SE3f();
    mbHasLastGoodPose = false;
    mpLastGoodReferenceKF = nullptr;
    mvIniMatches.clear();
...
bool Tracking::GetRiskImage(cv::Mat &out)
{
    if(!mbUsePatchDefense)
        return false;
    std::lock_guard<std::mutex> lock(mMutexRiskImage);
    if(!mbRiskImageReady || mLastRiskImage.empty())
        return false;
    mLastRiskImage.copyTo(out);
    return true;
}

bool Tracking::GetRiskMaskImage(cv::Mat &out)
{
    if(!mbUsePatchDefense)
        return false;
    std::lock_guard<std::mutex> lock(mMutexRiskMask);
    if(!mbRiskMaskReady || mLastRiskMaskImage.empty())
        return false;
    mLastRiskMaskImage.copyTo(out);
    return true;
}

void Tracking::UpdateRiskVisualization(const Frame &frame)
{
    if(frame.N==0 || frame.mvPatchWeights.empty())
    {
        std::lock_guard<std::mutex> lock(mMutexRiskImage);
        mLastRiskImage.release();
        mbRiskImageReady = false;
    ...
    cv::Mat vis(mImGray.rows, mImGray.cols, CV_8UC3, cv::Scalar::all(0));
    const int radius = 3;
    for(int i=0; i<frame.N; ++i)
    {
        if(i >= static_cast<int>(frame.mvPatchWeights.size()))
            continue;
        const cv::KeyPoint &kp = frame.mvKeysUn[i];
        const cv::Point center(cvRound(kp.pt.x), cvRound(kp.pt.y));
        if(center.x < 0 || center.y < 0 || center.x >= vis.cols || center.y >= vis.rows)
            continue;
        const float weight = frame.mvPatchWeights[i];
        const float risk = 1.f - std::min(std::max(weight, 0.f), 1.f);
        cv::Scalar color = RiskToColor(risk);
        cv::circle(vis, center, radius, color, -1, cv::LINE_AA);
    }

    {
        std::lock_guard<std::mutex> lock(mMutexRiskImage);
        vis.copyTo(mLastRiskImage);
        mbRiskImageReady = true;
    }
}

void Tracking::UpdateRiskMaskVisualization()
{
    if(!mpPatchDefense || !mbUsePatchDefense)
    {
        std::lock_guard<std::mutex> lock(mMutexRiskMask);
        mLastRiskMaskImage.release();
        mbRiskMaskReady = false;
    ...
    cv::Mat riskMask;
    if(!mpPatchDefense->GetRiskMask(riskMask) || riskMask.empty())
    {
        std::lock_guard<std::mutex> lock(mMutexRiskMask);
        mLastRiskMaskImage.release();
        mbRiskMaskReady = false;
    ...
    cv::Mat vis(riskMask.size(), CV_8UC3);
    for(int y=0; y<riskMask.rows; ++y)
    {
        const float* row = riskMask.ptr<float>(y);
        cv::Vec3b* out = vis.ptr<cv::Vec3b>(y);
        for(int x=0; x<riskMask.cols; ++x)
        {
            float r = std::min(std::max(row[x], 0.f), 1.f);
            const uchar red = static_cast<uchar>(255.f * r);
            const uchar green = static_cast<uchar>(255.f * (1.f - r));
            out[x] = cv::Vec3b(0, green, red);
        }
    }

    {
        std::lock_guard<std::mutex> lock(mMutexRiskMask);
        vis.copyTo(mLastRiskMaskImage);
        mbRiskMaskReady = true;
    }
}

cv::Scalar Tracking::RiskToColor(float risk) const
{
    risk = std::min(std::max(risk, 0.f), 1.f);
    const uchar red = static_cast<uchar>(255.f * risk);
    const uchar blue = static_cast<uchar>(255.f * (1.f - risk));
    const uchar green = static_cast<uchar>(128.f * (1.f - std::abs(risk - 0.5f)*2.f));
