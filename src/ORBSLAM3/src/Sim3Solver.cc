    ...
    pCamera1(pKF1->mpCamera), pCamera2(pKF2->mpCamera)
{
    ...
    for(int i1=0; i1<mN1; i1++)
    {
        if(vpMatched12[i1])
        {
            ...
            const float w1 = pKF1->GetKeypointWeight(indexKF1);
            const float w2 = pKFm->GetKeypointWeight(indexKF2);
            const float wij = bUseFrontendWeights ? std::max(std::min(w1,w2), cfg.w_min) : 1.f;
            mvMatchWeights.push_back(wij);
...
Eigen::Matrix4f Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers)
{
    ...
    while(mnIterations<mRansacMaxIts && nCurrentIterations<nIterations)
    {
        ...
        DrawWeightedSample(vSample);
        for(short i = 0; i < 3; ++i)
        {
            const size_t idx = vSample[i];
        ...
        if(mnInliersi>=mRansacMinInliers && mCurrentWeightedScore>mBestWeightedScore)
        {
            ...
            mBestScale = ms12i;
            mBestWeightedScore = mCurrentWeightedScore;
...
void Sim3Solver::ComputeSim3(Eigen::Matrix3f &P1, Eigen::Matrix3f &P2)
{
    ...
    const float q0 = evec(0, maxIndex);
    if(!std::isfinite(vecNorm) || !std::isfinite(q0) || vecNorm < 1e-8f)
    {
    ...
    else
    {
        ...
        const double ang = atan2(static_cast<double>(vecNorm), static_cast<double>(q0));

        vec = static_cast<float>(2.0 * ang) * vec / vecNorm; //Angle-axis representation. quaternion angle is the half
        if(!vec.allFinite())
            mR12i = Eigen::Matrix3f::Identity();
        else
...
void Sim3Solver::CheckInliers()
{
    ...
    for(size_t i=0; i<mvP1im1.size(); i++)
    {
        ...
        if(err1<mvnMaxError1[i] && err2<mvnMaxError2[i])
        {
            mvbInliersi[i]=true;
            mnInliersi++;
            if(i<mvMatchWeights.size())
                mCurrentWeightedScore += mvMatchWeights[i];
        }
        else
            mvbInliersi[i]=false;
    }
...
void Sim3Solver::DrawWeightedSample(std::vector<size_t> &indices)
{
    if(indices.empty())
        return;
    std::vector<size_t> available = mvAllIndices;
    for(size_t i=0; i<indices.size(); ++i)
    {
        ...
        float total = 0.f;
        for(size_t idx : available)
            total += (idx < mvMatchWeights.size()) ? mvMatchWeights[idx] : 1.f;

        size_t chosen = 0;
        if(total <= 0.f)
        {
            chosen = static_cast<size_t>(DUtils::Random::RandomInt(0, available.size()-1));
        }
        else
        {
            float r = DUtils::Random::RandomValue<float>() * total;
            float accum = 0.f;
            for(size_t pos=0; pos<available.size(); ++pos)
            {
                const size_t idx = available[pos];
                accum += (idx < mvMatchWeights.size()) ? mvMatchWeights[idx] : 1.f;
                if(accum >= r)
                {
                    chosen = pos;
                    break;
