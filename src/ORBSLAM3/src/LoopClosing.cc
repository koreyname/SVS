            ...
            if(bFindedRegion)
            {
                ...
                if(mbLoopDetected)
                {
                    ...
                    if (bGoodLoop) {
                        ...
                        const bool useLoopDefense = cfg.use_patch_defense && cfg.enable_loop_defense;
                        if(useLoopDefense)
                        {
...
void LoopClosing::SaveLoopMatchImage()
{
    ...
    KeyFrame* pMatchedKF = mpLoopMatchedKF ? mpLoopMatchedKF : mpMatchedKF;
    const std::vector<MapPoint*> &vpMatchedPoints = !mvpLoopMatchedMPs.empty() ? mvpLoopMatchedMPs : mvpCurrentMatchedPoints;
    if(!mpCurrentKF || !pMatchedKF || vpMatchedPoints.empty())
        return;

    cv::Mat imCur, imLoop;
    if(!mpCurrentKF->GetDebugImageGray(imCur) || !pMatchedKF->GetDebugImageGray(imLoop))
    {
    ...
    const int curNlim = (mpCurrentKF->NLeft != -1) ? mpCurrentKF->NLeft : mpCurrentKF->N;
    const int loopNlim = (pMatchedKF->NLeft != -1) ? pMatchedKF->NLeft : pMatchedKF->N;

    std::vector<cv::DMatch> matches;
    matches.reserve(static_cast<size_t>(curNlim));
    for(int i=0; i<curNlim && i < static_cast<int>(vpMatchedPoints.size()); ++i)
    {
        MapPoint* pMP = vpMatchedPoints[i];
        if(!pMP)
            continue;
        const int idxLoop = std::get<0>(pMP->GetIndexInKeyFrame(pMatchedKF));
        if(idxLoop < 0 || idxLoop >= loopNlim)
            continue;
        matches.emplace_back(i, idxLoop, 0.f);
    }
...
bool LoopClosing::EvaluateLoopSafety(float &safeRatio, double &klDiv, float &cover)
{
    ...
    const int loopN = pMatchedKF->N;
    for(int i=0; i<loopN; ++i)
    {
        const float w = pMatchedKF->GetKeypointWeight(i);
        if(w > cfg.safe_weight_threshold)
            loopSafeTotal++;
    }

    for(size_t i=0; i<vpMatchedPoints.size(); ++i)
    {
        MapPoint* pMP = vpMatchedPoints[i];
        if(!pMP)
            continue;
        ...
        const float wCur = mpCurrentKF->GetKeypointWeight(static_cast<int>(i));

        const int idxLoop = std::get<0>(pMP->GetIndexInKeyFrame(pMatchedKF));
        float wLoop = 1.f;
        if(idxLoop>=0)
        {
            wLoop = pMatchedKF->GetKeypointWeight(idxLoop);
        }
        else
        {
            ...
            for(const auto &obs : observations)
            {
                if(obs.first==mpCurrentKF)
                    continue;
                const int idx = std::get<0>(obs.second);
                if(idx<0)
                    continue;
                wLoop = obs.first->GetKeypointWeight(idx);
                break;
            }
        }
        ...
        if(bSafe)
        {
            ...
            if(idxLoop>=0)
            {
                accumulate(pMatchedKF, pMatchedKF->mvKeysUn[idxLoop], histLoop);
                loopHistSamples++;
            }
        }
    }
...
{  
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if(!mbStopGBA)
        {
            Verbose::PrintMess("Global Bundle Adjustment finished", Verbose::VERBOSITY_QUIET);
            Verbose::PrintMess("Updating map ...", Verbose::VERBOSITY_QUIET);
...
#endif
            Verbose::PrintMess("Map updated!", Verbose::VERBOSITY_QUIET);
        }
