...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        if(!std::isfinite(dirGNorm) || dirGNorm < 1e-8f)
        {
        ...
        else
        {
            ...
            if(std::isfinite(ang))
            {
                if(std::isfinite(nv) && nv >= 1e-8f)
                    vzg = v * (ang/nv);
                else if(cosg < -0.9999f)
                    vzg = 3.14159265358979323846f * Eigen::Vector3f(1.0f, 0.0f, 0.0f);
            }
