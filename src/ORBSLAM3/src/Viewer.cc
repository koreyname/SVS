...
void Viewer::Run()
{
    ...
    // Ensure the OpenCV frame window is closed before leaving the viewer thread.
    // Otherwise, if shutdown waits for backend tasks (e.g. GBA), desktop may mark
    // the stale window as "not responding" because no thread pumps HighGUI events.
