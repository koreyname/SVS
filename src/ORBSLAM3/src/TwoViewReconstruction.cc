...
namespace ORB_SLAM3
{
    ...
    bool TwoViewReconstruction::Reconstruct(const std::vector<cv::KeyPoint>& vKeys1, const std::vector<cv::KeyPoint>& vKeys2, const vector<int> &vMatches12,
                                             ...
                                             const std::vector<float> *pMatchWeights)
    {
        mvKeys1.clear();
        mvKeys2.clear();
        ...
        mvMatchWeights.reserve(mvKeys1.size());
        for(size_t i=0, iend=vMatches12.size();i<iend; i++)
        {
            if(vMatches12[i]>=0)
            {
                ...
                mvbMatched1[i]=true;
                const float w = (pMatchWeights && i<pMatchWeights->size()) ? (*pMatchWeights)[i] : 1.f;
                mvMatchWeights.push_back(w);
            }
            else
                mvbMatched1[i]=false;
        }
        ...
        for(int it=0; it<mMaxIterations; it++)
        {
            ...
            for(size_t j=0; j<8; j++)
            {
                ...
                for(size_t cand : vAvailableIndices)
                    total += (cand < mvMatchWeights.size()) ? mvMatchWeights[cand] : 1.f;
                ...
                else
                {
                    ...
                    for(size_t pos=0; pos<vAvailableIndices.size(); ++pos)
                    {
                        ...
                        accum += (cand < mvMatchWeights.size()) ? mvMatchWeights[cand] : 1.f;
                        if(accum >= r)
                        {
                            chosenPos = pos;
                            break;
