...
inline double FrontendWeightScale(const float weight)
{
    const PatchDefenseConfig &cfg = GetPatchDefenseConfig();
    if(!cfg.use_patch_defense || !cfg.enable_weighted_frontend)
        return 1.0;
    return static_cast<double>(std::max(weight, cfg.w_floor));
}

inline double BackendWeightScale(const float weight)
{
    const PatchDefenseConfig &cfg = GetPatchDefenseConfig();
    if(!cfg.use_patch_defense || !cfg.enable_weighted_ba)
        return 1.0;
    return static_cast<double>(std::max(weight, cfg.w_floor));
}
       ...
       const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(leftIndex != -1 && pKF->mvuRight[get<0>(mit->second)]<0)
            {
                ...
                const double wScale = BackendWeightScale(pKF->GetKeypointWeight(leftIndex));
                e->setInformation(Eigen::Matrix2d::Identity()*invSigma2*wScale);
            ...
            else if(leftIndex != -1 && pKF->mvuRight[leftIndex] >= 0) //Stereo observation
            {
                ...
                const float &invSigma2 = pKF->mvInvLevelSigma2[kpUn.octave];
                const double wScale = BackendWeightScale(pKF->GetKeypointWeight(leftIndex));
                Eigen::Matrix3d Info = Eigen::Matrix3d::Identity()*invSigma2*wScale;
            ...
            if(pKF->mpCamera2){
                const int rightIndexAll = get<1>(mit->second);
                int rightIndex = rightIndexAll;
                ...
                if(rightIndex != -1 && rightIndex < pKF->mvKeysRight.size()){
                    ...
                    const double wScale = BackendWeightScale(pKF->GetKeypointWeight(rightIndexAll));
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2*wScale);
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
        ...
        if(pMP)
        {
            ...
            if(!pFrame->mpCamera2){
                ...
                if(pFrame->mvuRight[i]<0)
                {
                    ...
                    const double wScale = FrontendWeightScale(pFrame->GetKeypointWeight(i));
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2*wScale);
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            const double wScale = FrontendWeightScale(pFrame->GetKeypointWeight(idx));
            const float chi2Unweighted = (wScale > 0.0) ? static_cast<float>(chi2 / wScale) : chi2;

            if(chi2Unweighted>chi2Mono[it])
            {                
        ...
        for(size_t i=0, iend=vpEdgesStereo.size(); i<iend; i++)
        {
            ...
            if(chi2Unweighted>chi2Stereo[it])
            {
                pFrame->mvbOutlier[idx]=true;
                e->setLevel(1);
                nBad++;
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                if(leftIndex != -1 && pKFi->mvuRight[get<0>(mit->second)]<0)
                {
                    ...
                    e->setMeasurement(obs);
                    const float &invSigma2 = pKFi->mvInvLevelSigma2[kpUn.octave];
                    const double wScale = BackendWeightScale(pKFi->GetKeypointWeight(leftIndex));
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2*wScale);
                ...
                if(pKFi->mpCamera2){
                    ...
                    if(rightIndex != -1 ){
                        ...
                        const double wScale = BackendWeightScale(pKFi->GetKeypointWeight(rightIndexAll));
                        e->setInformation(Eigen::Matrix2d::Identity()*invSigma2*wScale);
                            ...
                            const bool bFixScale, Eigen::Matrix<double,7,7> &mAcumHessian, const bool bAllPoints)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        float wEdge = pKF1->GetKeypointWeight(i);
        if(i2 >= 0)
            wEdge = std::min(wEdge, pKF2->GetKeypointWeight(i2));
        const double wScale = FrontendWeightScale(wEdge);
        e12->setInformation(Eigen::Matrix2d::Identity()*invSigmaSquare1*wScale);
        ...
        e21->setInformation(Eigen::Matrix2d::Identity()*invSigmaSquare2*wScale);
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit)
{
        ...
        for(int i=0; i<N; i++)
        {
            ...
            if(pMP)
            {
                ...
                else if(!bRight)
                {
                    ...
                    const float &invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave]/unc2;
                    const double wScale = FrontendWeightScale(pFrame->GetKeypointWeight(i));
                    e->setInformation(Eigen::Matrix3d::Identity()*invSigma2*wScale);
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            if((chi2Unweighted>chi2Mono[it]&&!bClose)||(bClose && chi2Unweighted>chi2close)||!e->isDepthPositive())
            {
                pFrame->mvbOutlier[idx]=true;
                e->setLevel(1);
                nBadMono++;
    ...
    if ((nInliers<30) && !bRecInit)
    {
        ...
        for(size_t i=0, iend=vnIndexEdgeMono.size(); i<iend; i++)
        {
            ...
            e1->computeError();
            const double wScale = FrontendWeightScale(pFrame->GetKeypointWeight(idx));
            const float chi2Unweighted = (wScale > 0.0) ? static_cast<float>(e1->chi2() / wScale) : static_cast<float>(e1->chi2());
            if (chi2Unweighted<chi2MonoOut)
                pFrame->mvbOutlier[idx]=false;
            else
                nBad++;
        }
        for(size_t i=0, iend=vnIndexEdgeStereo.size(); i<iend; i++)
        {
            ...
            e2->computeError();
            const double wScale = FrontendWeightScale(pFrame->GetKeypointWeight(idx));
            const float chi2Unweighted = (wScale > 0.0) ? static_cast<float>(e2->chi2() / wScale) : static_cast<float>(e2->chi2());
            if (chi2Unweighted<chi2StereoOut)
                pFrame->mvbOutlier[idx]=false;
            else
                nBad++;
        }
