...
public:
    ...
    // Patch defense buffers (risk + weight per keypoint).
    std::vector<float> mvPatchRisk;
    std::vector<float> mvPatchWeights;
    ...
    {
        if(idx>=mvPatchWeights.size())
            return 1.f;
        return mvPatchWeights[idx];
    }

    inline float GetKeypointRisk(const size_t idx) const
    {
        if(idx>=mvPatchRisk.size())
            return 0.f;
        return mvPatchRisk[idx];
    }
...
private:
    ...
    void InitializePatchDefenseBuffers();
