...
public:
    ...
    // Debug: for loop match visualization
    void SetDebugImageGray(const cv::Mat &imGray);
    bool GetDebugImageGray(cv::Mat &out) const;
    ...
    inline float GetKeypointWeight(const size_t idx) const
    {
        if(idx>=mKeypointWeights.size())
            return 1.f;
        return mKeypointWeights[idx];
    }

    inline float GetKeypointRisk(const size_t idx) const
    {
        if(idx>=mKeypointRisk.size())
            return 0.f;
        return mKeypointRisk[idx];
    }
...
public:
    ...
    const std::vector<float> mKeypointWeights;
    const std::vector<float> mKeypointRisk;
...
protected:
    ...
    // Debug image for loop match visualization (optional; not serialized)
    cv::Mat mDebugImageGray;
    mutable std::mutex mMutexDebugImage;
