...
vector<Geometry::DynKeyPoint> Geometry::ExtractDynPoints(const vector<ORB_SLAM2::Frame> &vRefFrames,
    ...
    for (int i(0); i < mnRefFrames; i++)
    {
        ...
        if(refFrame.N <= 0 || refFrame.mImDepth.empty())
            continue;
        ...
        for(int j(0); j < refFrame.N; j++){
            ...
            if(u < 0 || v < 0 || u >= refFrame.mImDepth.cols || v >= refFrame.mImDepth.rows)
                continue;
        ...
        for (int j(0); j < k; j++)
        {
            ...
            mp.at<float>(0,0) = vMPw.at<float>(0,j)/matInvDepthRefFrame.at<float>(j,0);
            mp.at<float>(1,0) = vMPw.at<float>(1,j)/matInvDepthRefFrame.at<float>(j,0);
            mp.at<float>(2,0) = vMPw.at<float>(2,j)/matInvDepthRefFrame.at<float>(j,0);
